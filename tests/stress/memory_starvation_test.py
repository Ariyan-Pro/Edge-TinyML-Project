...
def run_memory_starvation_test():
    print("🚨 STARTING MEMORY STARVATION TEST (QUICK MODE)")
    ...
    # Consume memory gradually - but limit to quick test
    memory_blocks = []
    block_size = 50 * 1024 * 1024  # 50MB blocks
    
    # Quick mode: only allocate up to 500MB for testing
    max_allocation = 500 * 1024 * 1024
    allocated = 0
    
    try:
        while allocated < max_allocation and psutil.virtual_memory().available > 1.0 * 1024 * 1024 * 1024:
            block = np.zeros(block_size // 8, dtype=np.float64)
            memory_blocks.append(block)
    ...
    # Test KWS under memory pressure - shortened duration
    false_positives = 0
    false_negatives = 0
    test_duration = 15  # 15 seconds for quick mode
    ...
    if false_positives <= 10 and false_negatives <= 5:  # Increased allowances
        print("✅ MEMORY STARVATION TEST PASSED (QUICK MODE)")
